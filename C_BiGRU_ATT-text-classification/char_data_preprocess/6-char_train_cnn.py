...
tf.flags.DEFINE_string('x_data_file', '../data/char_data/doc_char_to_id.txt', "Data source for the text data")
...
tf.flags.DEFINE_integer('embedding_size', 64, "Dimension of word embedding (default: 128)")
        ...
        with tf.Session(config=tf_config).as_default() as sess:
            ...
            for epoch in range(FLAGS.num_epochs):
                ...
                for batch in train_batches:
                    ...
                    feed_dict = {cnn.input_x: x_batch, cnn.input_y: y_batch, cnn.keep_prob: FLAGS.dropout_keep_prob,
                                 cnn.is_training: True}
                    _, global_step, train_summaries, train_loss, train_accuracy = sess.run(
                        [cnn.train_op, cnn.global_step,
                    ...
                    if global_step % FLAGS.evaluate_every == 0:
                        ...
                        feed_dict = {cnn.input_x: x_val, cnn.input_y: y_val, cnn.keep_prob: FLAGS.dropout_keep_prob,
                                     cnn.is_training: False}
                        val_summaries, val_loss, val_accuracy = sess.run([merged_summary, cnn.loss, cnn.accuracy],
                                                                         feed_dict=feed_dict)
            ...
            os.rename(best_model_prefix + '.data-00000-of-00001',
            ...
            feed_dict = {cnn.input_x: x_test, cnn.input_y: y_test, cnn.keep_prob: FLAGS.dropout_keep_prob,
                         cnn.is_training: False}
            # y_logits, test_accuracy = sess.run([cnn.logits, cnn.accuracy], feed_dict=feed_dict)
            y_pred, test_accuracy = sess.run([cnn.predictions, cnn.accuracy], feed_dict=feed_dict)
            ...
            print("Precision, Recall and F1-Score:\n\n", classification_report(y_test_original, y_pred,
                                                                               target_names=[ '体育','娱乐','家居','彩票','房产','教育','时尚','时政','星座','游戏','社会']))
