...
class TextRNN(object):
    ...
    def __init__(self, vocab_size, embedding_size, sequence_length, rnn_size, num_layers,
        ...
        with tf.name_scope('attention'):
            ...
            self.attention_output = tf.reduce_sum(rnn_output * tf.reshape(self.alpha, [-1, sequence_size, 1]), 1)

        # Add dropout
        with tf.name_scope('dropout'):
            # attention_output shape: (batch_size, hidden_size)
            self.final_output = tf.nn.dropout(self.attention_output, self.keep_prob)
