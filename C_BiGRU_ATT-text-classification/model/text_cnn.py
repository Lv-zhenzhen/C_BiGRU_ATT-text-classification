...
class TextCNN(object):
    ...
    def __init__(self, vocab_size, embedding_size, sequence_length, filter_sizes, num_filters,
        ...
        self.pooled_concat_flat = tf.squeeze(pooled_concat, [1, 2])

        # Add dropout
        with tf.name_scope('dropout'):
            # pooled_concat_flat shape: (batch_size, num_filters * len(filter_sizes))
            self.final_output = tf.contrib.layers.dropout(self.pooled_concat_flat, keep_prob=self.keep_prob, is_training=self.is_training)
